...
Write-Output "applicationHost.config HTTPS updated"

# Adding Secueiry Header rules in config file
$ConfigFile = "C:\Windows\System32\inetsrv\config\applicationHost.config"
$content = [System.IO.File]::ReadAllText($ConfigFile).Replace('<httpProtocol>
            <customHeaders>
                <clear />
		...
		'<httpProtocol>
            <customHeaders>
                ...
                <add name="Content-Security-Policy" value="default-src ‘https’; base-uri ‘none’;" />
                ...
                <add name="Cross-Origin-Embedder-Policy" value="(unsafe-none|require-corp); report-to=default" />
                <add name="Cross-Origin-Opener-Policy" value="(same-origin|same-origin-allow-popups|unsafe-none); report-to=default" />
                <add name="Cross-Origin-Resource-Policy" value="(same-site|same-origin|cross-origin)" />
...
[System.IO.File]::WriteAllText($ConfigFile, $content)
Write-Output "applicationHost.config HostHeader updated"


New-Item -ItemType File -Path C:\inetpub\wwwroot\web.config -Value "TestFile"
$ConfigFile = "C:\inetpub\wwwroot\web.config"
$content = [System.IO.File]::ReadAllText($ConfigFile).Replace('TestFile',
...
[System.IO.File]::WriteAllText($ConfigFile, $content)
Write-Output "Creating Web.Config File with server header removed"
